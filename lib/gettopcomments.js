  ...
  module.exports = function(id, next) {
    ...
    request({
      ...
      for (var i=0; i<count; i++) {
      	...
      	if (type === 'text'){
      		//set media as null
      		comments[i] = {text: (json.payload.comments[i].text), media:null};
      	}
      	//comment contains media (image/gif/video)
      		...
      		if (mediaType === 'STATIC'){ 
      			if (mediaText != ''){
      				...
      				comments[i] = {text: mediaText, media: (json.payload.comments[i].media[0].imageMetaByType.image.url)};
      			}else
      				comments[i] = {text: null, media: (json.payload.comments[i].media[0].imageMetaByType.image.url)};

      		}else if (mediaType === 'ANIMATED'){
      			if (mediaText != ''){
					//media is gif/video
					comments[i] = {text: mediaText, media: (json.payload.comments[i].media[0].imageMetaByType.video.url)};
      			}else
      				comments[i] = {text: null, media: (json.payload.comments[i].media[0].imageMetaByType.video.url)};
      		}
      	}
      }
